...
interface VeranstaltungenDialogProps {
  ...
  defaultValues?: Omit<Veranstaltungen['fields'], 'veranstaltungstyp'> & {
    veranstaltungstyp?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  veranstaltungstyp: LOOKUP_OPTIONS['veranstaltungen']?.['veranstaltungstyp'] ?? [],
};
...
export function VeranstaltungenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: VeranstaltungenDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "veranstaltungsname": string | null, // Veranstaltungsname\n  "veranstaltungstyp": LookupValue | null, // Veranstaltungstyp (select one key: "stammtisch" | "hands_on_meeting" | "vortrag") mapping: stammtisch=Stammtisch, hands_on_meeting=Hands-on Meeting, vortrag=Vortrag\n  "datum_uhrzeit": string | null, // YYYY-MM-DDTHH:MM\n  "veranstaltungsort": string | null, // Veranstaltungsort\n  "beschreibung": string | null, // Beschreibung\n  "notizen": string | null, // Notizen\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'veranstaltungsname': (
      <div key="veranstaltungsname" className="space-y-1.5">
        <Label htmlFor="veranstaltungsname">Veranstaltungsname</Label>
        <Input
          id="veranstaltungsname"
          placeholder="z. B. AI-Stammtisch Juli"
    ...
    'veranstaltungstyp': (
      <div key="veranstaltungstyp" className="space-y-1.5">
        <Label htmlFor="veranstaltungstyp">Veranstaltungstyp</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.veranstaltungstyp) === 'stammtisch'}
            onClick={() => setFields(f => ({ ...f, veranstaltungstyp: (lookupKey(f.veranstaltungstyp) === 'stammtisch' ? undefined : 'stammtisch') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.veranstaltungstyp) === 'stammtisch'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.veranstaltungstyp) === 'hands_on_meeting'}
            onClick={() => setFields(f => ({ ...f, veranstaltungstyp: (lookupKey(f.veranstaltungstyp) === 'hands_on_meeting' ? undefined : 'hands_on_meeting') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.veranstaltungstyp) === 'hands_on_meeting'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.veranstaltungstyp) === 'vortrag'}
            onClick={() => setFields(f => ({ ...f, veranstaltungstyp: (lookupKey(f.veranstaltungstyp) === 'vortrag' ? undefined : 'vortrag') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.veranstaltungstyp) === 'vortrag'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'veranstaltungsort': (
      <div key="veranstaltungsort" className="space-y-1.5">
        ...
        <Input
          ...
          placeholder="z. B. Café im Zentrum"
    ...
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        ...
        <Textarea
          id="beschreibung"
          placeholder="Thema, Gast, Agenda, Einlass..."
    ...
    'notizen': (
      <div key="notizen" className="space-y-1.5">
        ...
        <Textarea
          id="notizen"
          placeholder="Besonderheiten, Vor-Ort-Infos..."
  ...
  const FIELD_LABELS: Record<string, string> = {"veranstaltungsname": "Veranstaltungsname", "veranstaltungstyp": "Veranstaltungstyp", "datum_uhrzeit": "Datum und Uhrzeit", "veranstaltungsort": "Veranstaltungsort", "beschreibung": "Beschreibung", "notizen": "Notizen"};
