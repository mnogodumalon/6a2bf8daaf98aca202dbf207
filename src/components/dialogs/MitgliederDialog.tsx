...
export function MitgliederDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: MitgliederDialogProps) {
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
      ...
      <div key="mitgliedsstatus" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.mitgliedsstatus) === 'normales_mitglied'}
            onClick={() => setFields(f => ({ ...f, mitgliedsstatus: (lookupKey(f.mitgliedsstatus) === 'normales_mitglied' ? undefined : 'normales_mitglied') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.mitgliedsstatus) === 'normales_mitglied'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.mitgliedsstatus) === 'foerdermitglied'}
            onClick={() => setFields(f => ({ ...f, mitgliedsstatus: (lookupKey(f.mitgliedsstatus) === 'foerdermitglied' ? undefined : 'foerdermitglied') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.mitgliedsstatus) === 'foerdermitglied'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.mitgliedsstatus) === 'korrespondenzmitglied'}
            onClick={() => setFields(f => ({ ...f, mitgliedsstatus: (lookupKey(f.mitgliedsstatus) === 'korrespondenzmitglied' ? undefined : 'korrespondenzmitglied') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.mitgliedsstatus) === 'korrespondenzmitglied'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'austrittsdatum': (
      <div key="austrittsdatum" className="space-y-1.5">
        ...
        <DatePicker
          id="austrittsdatum"
          placeholder="Austrittsdatum (falls relevant)"
          mode="date"
    ...
    'notizen': (
      <div key="notizen" className="space-y-1.5">
        ...
        <Textarea
          id="notizen"
          placeholder="Besonderheiten, Interessen, Kontakt-Notizen..."
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "mitgliedsstatus": "Mitgliedsstatus", "eintrittsdatum": "Eintrittsdatum", "austrittsdatum": "Austrittsdatum", "notizen": "Notizen"};
