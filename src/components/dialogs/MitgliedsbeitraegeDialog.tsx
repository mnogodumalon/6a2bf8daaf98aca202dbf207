...
import type { Mitgliedsbeitraege, Mitglieder, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Mitgliedsbeitraege';
...
interface MitgliedsbeitraegeDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Mitgliedsbeitraege['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<Mitgliedsbeitraege['fields'], 'zahlungsart' | 'zahlungsstatus'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  zahlungsart: LOOKUP_OPTIONS['mitgliedsbeitraege']?.['zahlungsart'] ?? [],
  zahlungsstatus: LOOKUP_OPTIONS['mitgliedsbeitraege']?.['zahlungsstatus'] ?? [],
};
...
export function MitgliedsbeitraegeDialog({ open, onClose, onSubmit, defaultValues, recordId, mitgliederList, enablePhotoScan = true, enablePhotoLocation = true }: MitgliedsbeitraegeDialogProps) {
  const [fields, setFields] = useState<Partial<Mitgliedsbeitraege['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Mitgliedsbeitraege['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'mitgliedsbeitraege');
      await onSubmit(clean as Mitgliedsbeitraege['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "zahlungsart": LookupValue | null, // Zahlungsart (select one key: "lastschrift" | "bar" | "ueberweisung") mapping: lastschrift=Lastschrift, bar=Bar, ueberweisung=Überweisung\n  "zahlungsstatus": LookupValue | null, // Zahlungsstatus (select one key: "bezahlt" | "ausstehend" | "gemahnt") mapping: bezahlt=Bezahlt, ausstehend=Ausstehend, gemahnt=Gemahnt\n  "notizen": string | null, // Notizen\n  "mitglied": string | null, // Display name from Mitglieder (see <available-records>)\n  "beitragsjahr": number | null, // Beitragsjahr\n  "betrag": number | null, // Betrag (€)\n  "zahlungsdatum": string | null, // YYYY-MM-DD\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Mitgliedsbeitraege['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Mitgliedsbeiträge bearbeiten' : 'Mitgliedsbeiträge hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
      ...
      <div key="zahlungsstatus" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.zahlungsstatus) === 'ausstehend'}
            onClick={() => setFields(f => ({ ...f, zahlungsstatus: (lookupKey(f.zahlungsstatus) === 'ausstehend' ? undefined : 'ausstehend') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.zahlungsstatus) === 'ausstehend'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'notizen': (
      <div key="notizen" className="space-y-1.5">
        ...
        <Textarea
          id="notizen"
          placeholder="Referenznummer, Besonderheiten..."
  ...
  const FIELD_LABELS: Record<string, string> = {"zahlungsart": "Zahlungsart", "zahlungsstatus": "Zahlungsstatus", "notizen": "Notizen", "mitglied": "Mitglied", "beitragsjahr": "Beitragsjahr", "betrag": "Betrag (€)", "zahlungsdatum": "Zahlungsdatum"};
  const CURRENCY_KEYS = new Set<string>(["betrag"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"mitglied": {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "mitgliedsstatus": "Mitgliedsstatus", "eintrittsdatum": "Eintrittsdatum", "austrittsdatum": "Austrittsdatum", "notizen": "Notizen"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.MITGLIEDSBEITRAEGE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
