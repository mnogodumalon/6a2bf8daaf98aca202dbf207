...
import { MitgliedsbeitraegeViewDialog } from '@/components/dialogs/MitgliedsbeitraegeViewDialog';
...
const MITGLIEDER_FIELDS = [
  ...
  { key: 'mitgliedsstatus', label: 'Mitgliedsstatus', type: 'lookup/radio', options: [{ key: 'normales_mitglied', label: 'Normales Mitglied' }, { key: 'foerdermitglied', label: 'Fördermitglied' }, { key: 'korrespondenzmitglied', label: 'Korrespondenzmitglied' }] },
  { key: 'eintrittsdatum', label: 'Eintrittsdatum', type: 'date/date' },
  { key: 'austrittsdatum', label: 'Austrittsdatum', type: 'date/date' },
  { key: 'notizen', label: 'Notizen', type: 'string/textarea' },
];
const VERANSTALTUNGEN_FIELDS = [
  { key: 'veranstaltungsname', label: 'Veranstaltungsname', type: 'string/text' },
  { key: 'veranstaltungstyp', label: 'Veranstaltungstyp', type: 'lookup/radio', options: [{ key: 'stammtisch', label: 'Stammtisch' }, { key: 'hands_on_meeting', label: 'Hands-on Meeting' }, { key: 'vortrag', label: 'Vortrag' }] },
...
const MITGLIEDSBEITRAEGE_FIELDS = [
  { key: 'zahlungsart', label: 'Zahlungsart', type: 'lookup/select', options: [{ key: 'lastschrift', label: 'Lastschrift' }, { key: 'bar', label: 'Bar' }, { key: 'ueberweisung', label: 'Überweisung' }] },
  { key: 'zahlungsstatus', label: 'Zahlungsstatus', type: 'lookup/radio', options: [{ key: 'bezahlt', label: 'Bezahlt' }, { key: 'ausstehend', label: 'Ausstehend' }, { key: 'gemahnt', label: 'Gemahnt' }] },
  { key: 'notizen', label: 'Notizen', type: 'string/textarea' },
...
const ENTITY_TABS = [
  ...
  { key: 'mitgliedsbeitraege', label: 'Mitgliedsbeiträge', pascal: 'Mitgliedsbeitraege' },
] as const;
...
export default function AdminPage() {
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'mitgliedsbeitraege': return (data as any).mitgliedsbeitraege as Mitgliedsbeitraege[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'mitgliedsbeitraege' && fieldKey === 'mitglied') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'mitgliedsbeitraege': return MITGLIEDSBEITRAEGE_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'mitgliedsbeitraege': return {
        create: (fields: any) => LivingAppsService.createMitgliedsbeitraegeEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateMitgliedsbeitraegeEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteMitgliedsbeitraegeEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'mitgliedsbeitraege' || dialogState?.entity === 'mitgliedsbeitraege') && (
        <MitgliedsbeitraegeDialog
          open={createEntity === 'mitgliedsbeitraege' || dialogState?.entity === 'mitgliedsbeitraege'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'mitgliedsbeitraege' ? handleUpdate : (fields: any) => handleCreate('mitgliedsbeitraege', fields)}
          defaultValues={dialogState?.entity === 'mitgliedsbeitraege' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'mitgliedsbeitraege' && (
        <MitgliedsbeitraegeViewDialog
          open={viewState?.entity === 'mitgliedsbeitraege'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'mitgliedsbeitraege', record: r }); }}
          mitgliederList={(data as any).mitglieder ?? []}
        />
      )}
