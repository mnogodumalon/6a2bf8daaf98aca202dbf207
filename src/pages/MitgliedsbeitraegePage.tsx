...
export default function MitgliedsbeitraegePage() {
  ...
  const [records, setRecords] = useState<Mitgliedsbeitraege[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Mitgliedsbeitraege | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Mitgliedsbeitraege | null>(null);
  ...
  async function handleCreate(fields: Mitgliedsbeitraege['fields']) {
  ...
  async function handleUpdate(fields: Mitgliedsbeitraege['fields']) {
    ...
    await LivingAppsService.updateMitgliedsbeitraegeEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteMitgliedsbeitraegeEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Mitgliedsbeiträge im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Mitgliedsbeiträge suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/mitgliedsbeitraege/${record.record_id}`); }}>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={8} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Mitgliedsbeiträge. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
