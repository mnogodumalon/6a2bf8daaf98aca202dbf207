...
import type { Mitglieder, Veranstaltungen, Mitgliedsbeitraege, CreateMitglieder, CreateVeranstaltungen, CreateMitgliedsbeitraege } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- MITGLIEDSBEITRAEGE ---
  static async getMitgliedsbeitraege(): Promise<Mitgliedsbeitraege[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.MITGLIEDSBEITRAEGE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Mitgliedsbeitraege[];
    return enrichLookupFields(records, 'mitgliedsbeitraege');
  }
  static async getMitgliedsbeitraegeEntry(id: string): Promise<Mitgliedsbeitraege | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.MITGLIEDSBEITRAEGE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Mitgliedsbeitraege;
    return enrichLookupFields([record], 'mitgliedsbeitraege')[0];
  }
  static async createMitgliedsbeitraegeEntry(fields: CreateMitgliedsbeitraege) {
    return callApi('POST', `/apps/${APP_IDS.MITGLIEDSBEITRAEGE}/records`, { fields: cleanFieldsForApi(fields as any, 'mitgliedsbeitraege') });
  }
  static async updateMitgliedsbeitraegeEntry(id: string, fields: Partial<CreateMitgliedsbeitraege>) {
    return callApi('PATCH', `/apps/${APP_IDS.MITGLIEDSBEITRAEGE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'mitgliedsbeitraege') });
  }
  static async deleteMitgliedsbeitraegeEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.MITGLIEDSBEITRAEGE}/records/${id}`);
  }
